...
{
    public class EditTMProColorClip : ChildUIElementsWindow
    {
        ...
        public override ChildUIElementsWindow Init(EditorWindow parentWindow)
        {
            ...
            elementUXML = parentVisualElement.Query<Foldout>("EditTMProColorClip", EditorToolsCore.ToggleableGroup);
        ...
        void UpdateDisplay()
        {
            ...
            for (int i = 0; i < TimelineEditor.selectedClips.Length; i++) {
                if (TimelineEditor.selectedClips[i].asset is TMProColorClip) {
        ...
        public static Color GetInitialValueFromSelection(TimelineClip[] clipSelection)
        {
            ...
            for (int i = 0; i < clipSelection.Length; i++) {
                if (clipSelection[i].asset is TMProColorClip) {
                    value = (clipSelection[i].asset as TMProColorClip).template.initialColor;
                    break;
                }
            }
        ...
        public static TimelineClip[] SetInitialValue(TimelineClip[] clipSelection, Color targetValue)
        {
            ...
            for (int i = 0; i < clipSelection.Length; i++) {
                if (clipSelection[i].asset is TMProColorClip) {
                    ...
                    TMProColorClip clipAsset = clipSelection[i].asset as TMProColorClip;
        ...
        public static Color GetTargetValueFromSelection(TimelineClip[] clipSelection)
        {
            ...
            for (int i = 0; i < clipSelection.Length; i++) {
                if (clipSelection[i].asset is TMProColorClip) {
                    value = (clipSelection[i].asset as TMProColorClip).template.targetColor;
                    break;
                }
            }
